...
export default function Features() {
  ...
  return (
    <section data-scroll data-scroll-section data-scroll-speed="0" className="py-24">
      <div className="container flex flex-col items-center justify-center">
        <h2 className="text-6xl font-medium mt-6 text-center max-w-xl mx-auto">
          Questions? We've got <span className="text-lime-400">answers</span>
        </h2>

        <div className="mt-12 flex flex-col gap-6 w-full max-w-2xl mx-auto">
          {faqs.map((faqs, itemIndex) => (
            <div
              key={faqs.question}
              className="bg-[#c5dff8] rounded-2xl border-b-gray-900/10 p-6"
            >
              <div
                className="flex justify-between items-center"
                onClick={() => setSelectedIndex(itemIndex)}
              >
                <h3 className="font-medium">{faqs.question}</h3>
                <svg
                  ...
                  className={twMerge(
                    ...
                    selectedIndex == itemIndex && "rotate-45"
                  )}
                >
              ...
              <AnimatePresence>
                {selectedIndex === itemIndex && (
                  ...
                  >
                    <p className="text-gray-500">{faqs.answer}</p>
                  </motion.div>
                )}
              </AnimatePresence>
            </div>
